...
async function boot() {
  renderChoices("types", TYPES, "type");
  renderChoices("areas", AREAS, "area");
...
function renderChoices(containerId, options, key) {
  ...
  options.forEach((option) => {
    ...
    button.addEventListener("click", () => {
      state[key] = option.id;
      renderChoices(containerId, options, key);
      renderRanking();
...
function renderRanking() {
  ...
  const rows = state.districts
    .map((d) => ({ ...d, median: medianFor(d) }))
    .filter((d) => d.median !== undefined)
  ...
  const max = rows[rows.length - 1].median;
  ...
  rows.forEach((row) => {
    ...
    const width = Math.round((row.median / max) * 100);

    tr.innerHTML = `
      <td>${row.name_fr || row.code}<span class="korean">${row.name}</span></td>
      <td>
        <span class="price">${won(row.median)}</span><br>
        <span class="euro">${eur(row.median)}</span>
      </td>
...
// d'etape de build ni de 200 Ko de JavaScript a telecharger.
